# Global {{AGENT_NAME}} Working Agreements
...
- Prefer repo-local `{{REPO_INSTRUCTIONS_FILE}}`, scripts, package managers, helpers, and naming conventions.
...
<!-- /target:codex -->
<!-- target:claude -->
- On Windows, programmatic command execution should resolve `npm.cmd`, `npx.cmd`, and `claude.cmd` safely; the Bash tool runs Git Bash, so set `MSYS_NO_PATHCONV=1` when an argument must keep a leading `/`.
<!-- /target:claude -->
...
<!-- /target:codex -->
<!-- target:claude -->
- Prefer one subagent and normally use no more than four in a single task; the bundled `agentchef:<role>` subagents are namespaced and never replace your own `~/.claude/agents/` definitions.
<!-- /target:claude -->
...
- Use the current prompt for one-off constraints and `{{REPO_INSTRUCTIONS_FILE}}` for durable working agreements.
<!-- target:codex -->
...
<!-- /target:codex -->
<!-- target:claude -->
- Use `~/.claude/settings.json` for permissions, hooks, environment, and model defaults; use `claude mcp add --scope user` (stored in `.claude.json`) for user-wide MCP servers; use `~/.claude/rules/` for durable user-wide guidance such as this file.
<!-- /target:claude -->
...
<!-- /target:codex -->
<!-- target:claude -->
- Use `CLAUDE.local.md` only for personal, gitignored project preferences and keep team instructions in the tracked `CLAUDE.md`.
<!-- /target:claude -->
...
- Prefer official {{VENDOR_DOCS}} for {{AGENT_NAME}} behavior and official project docs or Context7 for version-sensitive APIs.
...
- The installed {{ROUTING_SKILL}} skill contains the deferred routing ID catalog, specialist ownership map, skill compatibility aliases, MCP guidance, UI standards, and expanded operating notes.
