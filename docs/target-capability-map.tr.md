...
| Uzman içindeki MCP araçları | tanımlı her sunucu, rolün onay kurallarına tabi | subagent `tools:` listesi bir izin listesidir; rol yalnızca kendi bildirdiği `mcp__<sunucu>` girdilerine erişir. Talimatları Context7 veya Serena köprüsüne dayanan roller bunları bildirir, başka hiçbir şey verilmez | eşleşti, Claude'da bilinçli olarak daha dar |
...
| Uzman ajanlar | `~/.codex/agents/*.toml` (32 rol dosyası) | `plugins/agentchef-workflows/agents/*.md` altında `agentchef:<role>` plugin subagent'ları; `~/.claude/agents/` dokunulmaz | eşleniyor, ad-alanlı |
